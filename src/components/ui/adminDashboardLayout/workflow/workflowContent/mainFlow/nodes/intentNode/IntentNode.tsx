...
import IntentNodeIcon from "/assets/dashboard/main_dashboard/admin/workflow/Bot.svg"
...
const IntentNode = ({ ...data }) => {
    ...
    const onNodeAddClick: any = useCallback((data: any, selectedItem: any, nodeProps?: any) => {
        ...
        if ((selectedItem == "Intent" || selectedItem == "Transfer") && sourceNode.type != 'General') {
            ...
            if (nNode) {
                console.log("new node intent", nNode,selectedItem);
                setTimeout(() => {
    ...
    return (
        <div className={`${style.node_area} ${style.intent} ${(contextWorkflow.selectedNodeForSetting.id == data.id) ? style.recent_node : null}`}>
            <span className={style.message}><img src={MessageIcon} alt="" /></span>
            <div className={`${style.button} d-flex align-items-center`}>
                <img src={IntentNodeIcon} alt="" />
