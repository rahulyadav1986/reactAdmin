...
import GeneralNodeIcon from "/assets/dashboard/main_dashboard/admin/workflow/general.svg"
...
const ModuleNode = ({ ...data }) => {
  ...
  const settingsClickHandler = useCallback(() => {
    ...
    //  console.log(rInstance.getNode(data.id));
    rInstance.setNodes((nodes) => {
      nodes.map((nds) => {
        if (nds.id == data.id) {
  ...
  const moduleClickHandler = useCallback(() => {
    if (contextWorkFlow.selectedFlowRunningType == "") {
      if (data.data != undefined && data.data.onModuleClick != undefined) {
        data.data.onModuleClick(data.id);
      }
      else {
        rInstance.setNodes((nodes) => {
          nodes.map((nds) => {
            if (nds.id == data.id) {
              nds.data.toOpenModuleClick = data.id;
              console.log(nds);
              contextWorkFlow.handleWorkflowSettings(nds);
            }
          });
  ...
  // useEffect(() => {
  //   console.log("Module data changed", data.data);
  // }, [data]);

  return (
    <div className={`${style.node_area} ${style.module} ${(contextWorkFlow.selectedNodeForSetting.id == data.id) ? style.recent_node : null}`}>
      <span className={style.message}><img src={MessageIcon} alt="" /></span>
      <div className={`${style.button} d-flex align-items-center`} onClick={moduleClickHandler}>
      ...
      {contextWorkFlow.selectedFlowRunningType == '' && <span className={`${style.settings_area} d-flex align-items-center justify-content-between`}>
        <span className={style.module_text}
        >Module</span>
