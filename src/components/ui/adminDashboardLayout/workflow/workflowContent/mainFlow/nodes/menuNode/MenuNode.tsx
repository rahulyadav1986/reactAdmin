...
import MenuNodeIcon from "/assets/dashboard/main_dashboard/admin/workflow/nodeOption/menu.svg"
...
import { Handle, MarkerType, Position, useReactFlow, Node, getConnectedEdges, getOutgoers } from 'reactflow'
...
const MenuNode = ({ ...data }) => {
  ...
  const nodeAddClick = (selectedItem: any) => {
    ...
    console.log("out info 2", rInstance.getEdges());
  ...
  return (
    <div className={`${style.node_area} ${style.menu} ${(contextWorkflow.selectedNodeForSetting.id == data.id) ? style.recent_node : null}`}>
