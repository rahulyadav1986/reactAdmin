...
import MenuNodeIcon from "/assets/dashboard/main_dashboard/admin/workflow/menu.svg"
...
const TriggerNode = ({ ...data }) => {
  ...
  const onNodeAddClick: any = useCallback((data: any, selectedItem: any, nodeProps?: any) => {
    ...
    if ((selectedItem == "Intent" || selectedItem == "Transfer") && sourceNode.type != 'General') {
      ...
      if (selectedItem) {
        ...
        let runningWorkflowInfo:any  = contextWorkflow.runningWorkflowInfo;
        if(runningWorkflowInfo)
        {
          runningWorkflowInfo = JSON.parse(runningWorkflowInfo);
          if(runningWorkflowInfo.connectedWith == undefined || runningWorkflowInfo.connectedWith == null) {
        
            runningWorkflowInfo.type = selectedItem;
          }
  ...
  const onPointerClickHandler = (event: any) => {
    const sourceNode: any = rInstance.getNode(data.id);
  ...
  return (
    <div className={`${style.node_area} ${style.start} ${(contextWorkflow.selectedNodeForSetting.id == data.id) ? style.recent_node : null}`}>
      ...
      <Handle type="source" position={Position.Right} isConnectable={true} style={{ background: 'rgba(178, 193, 231, 1)', cursor: 'pointer' }} ref={optionRef}>
        ...
        {nodeOptionPop ? <NodeOptionPopup onMenuSelect={nodeAddClick} nodeType="Trigger" /> : null}
      </Handle>
